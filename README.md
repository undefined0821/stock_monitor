...
NEW:停更一小段时间，最近在憋好玩的：追涨杀跌，个人情绪影响操作。在用不同的模型测试交易策略，调自选股数据，跟着机器操作。需要测半个月以上的时间，看看机器交易策略的盈亏情况。
...
- **🔍 选股池（每日 14:25 自动扫描）**：全市场主板扫描取 TopN 推荐，逻辑加密封装（不在源码/前端暴露明细）；实时行情强预筛 + 本地日线库预热后零网络秒级出结果⚠这个功能还有点问题，K线数据不好拉，仅供娱乐
- **🗄 本地日线库**：每交易日 15:05 自采当日完整日线落盘，供回测按日期回溯；体积上限 + 保留 250 天双阈值自动清理（需要10d+才能有足够的K线样本数据）
